...
function handleProfileSubMenu (handlePage) {
  ...
  if (subMenu) {
    ...
    state.profileMenu.forEach( item => {
...
export function topMenu(handlePage, user) {
  ...
  listMenu.innerHTML += state.icons.menu;
  ...
  nameUser.textContent = "nome do usuário";
  profileMenu.appendChild(nameUser);
  profileMenu.innerHTML += state.icons.profile;  
