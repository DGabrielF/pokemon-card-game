const state = {
  view: {
    ...
    fieldCard: {
      ...
      playerAttributeValue: document.querySelector("#your-poke-attribute"),
      enemy: document.querySelector(".enemy-poke-card"),
      enemyAttributeValue: document.querySelector("#enemy-poke-attribute"),
    },
    cardAreas: {
    ...
    turn: {
      player: document.querySelector(".your-turn div"),
      enemy: document.querySelector(".enemy-turn div"),
    },
    attribute: {
...
function createCards(pokelist, cardArea, cardClass) {
  for (let i = 0; i < pokelist.length; i++) {
    ...
    pokelist[i].types.forEach((type) => {
      if (pokelist[i].types.length === 1) {
        card.classList.add(type);
      }
      else if (pokelist[i].types.length > 1 && !["normal","dragon","flying","poison"].includes(type)) {
        card.classList.add(type);
      } 
    });
...
function handleTurn() {
  ...
  state.view.fieldCard.playerAttributeValue.style.visibility = "hidden";
  state.view.fieldCard.enemyAttributeValue.style.visibility = "hidden";
...
function cleanRound() {
  ...
  const playerFieldToClean = document.querySelector(".duel-box .your-card")
  ...
  const enemyFieldToClean = document.querySelector(".duel-box .your-card")
  enemyFieldToClean.classList = "enemy-poke-card"
  enemyFieldToClean.innerHTML = ""
  state.action.pokemonSelector.disabled = false;
  
  state.view.fieldCard.playerAttributeValue.style.backgroundColor = "#00000030";
  state.view.fieldCard.enemyAttributeValue.style.backgroundColor = "#00000030";
}
...
function comparison() {
  state.view.fieldCard.playerAttributeValue.textContent = state.values.playerPokemonInField[state.values.attributeSelected];
  state.view.fieldCard.playerAttributeValue.style.visibility = "visible";
  state.view.fieldCard.enemyAttributeValue.textContent = state.values.enemyPokemonInField[state.values.attributeSelected];
  state.view.fieldCard.enemyAttributeValue.style.visibility = "visible";
  let isEndGame;
  if (state.values.playerPokemonInField[state.values.attributeSelected] > state.values.enemyPokemonInField[state.values.attributeSelected]) {
    state.view.fieldCard.playerAttributeValue.style.backgroundColor = "#00ff0080";
    ...
    state.view.fieldCard.enemyAttributeValue.style.backgroundColor = "#00ff0080";
